...
fig_TS_S2 <- ggplot(subset(S2_table, x %in% n_max$x  & y %in% n_max$y ))+
  ...
  geom_line(aes( x= Date, y=STR), 
            ...
            alpha=0.1)+
  # add NDVI
  geom_point(aes( x= Date, y=NDVI*30), # scale NDVI to show it on one plot with STR
             color= "#8B9A46", # show NDVI with green colour
             size=3.5, 
             alpha=0.9)+
  geom_line(aes( x= Date, y=NDVI*30), 
            color= "#8B9A46", 
            size=2, 
            alpha=0.1)+
  # Set the axis
  ...
  scale_y_continuous(name = "STR", 
                     ...
                     sec.axis = sec_axis(~./30, name= "NDVI"))+
  # theme parameters
  theme_bw()+
...
minimal_ndvi <- 0 # min NDVI value 
maximal_ndvi <- 0.5 # max NDVI value 
...
fig_TS_WTD <- ggplot(WTD_table, aes(x = Date, y = WTD))+
  ...
  scale_x_date(date_breaks = "1 month", date_labels = "%m/%Y",
               limits = c(as.Date("1/05/2018", format = "%d/%m/%Y"),
                      as.Date("1/10/2018", format = "%d/%m/%Y")))
fig_TS_WTD
...
# correlation between OPTRAM and WTD
data_table_cor<-data_table%>%
...
fig_TS_WTD+
  ...
  geom_segment( data = subset(data_table, x %in% data_table_bestcor$x  & y %in% data_table_bestcor$y),
                ...
                color = "#FF165D")+
  scale_y_continuous(name = "STR", 
                     sec.axis = sec_axis((~./180 + 105/180), name= "OPTRAM"))+
...
# We will create a set of maps of OPTRAM for the dates when "best pixel" is present. 
# First, derive the dates when "best pixel" has OPTRAM values
bextpixel_table <- data_table %>% filter(x %in% data_table_bestcor$x  & 
                                           y %in% data_table_bestcor$y)%>%
  drop_na()%>%
  select(Date)
bextpixel_table

# From data_table select only days that are present in bextpixel_table
data_table_reduced <- left_join(bextpixel_table, data_table, by = "Date")
data_table_reduced <- na.omit(data_table_reduced)

ggplot()+
  geom_raster(data=data_table_reduced, aes(x=x, y=y, fill=OPTRAM))+ 
  coord_equal()+
  ...
  geom_point(data = subset(data_table_reduced, x %in% data_table_bestcor$x  & 
                             y %in% data_table_bestcor$y),
             aes(x = x, y = y),
             shape = 5,
             color = "#F900BF",
